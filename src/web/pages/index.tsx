...
function Index() {
  ...
  useEffect(() => {
    if (gameState === "finished" && user?.username && results.length > 0 && !hasRecordedResult.current) {
      ...
      <div className="min-h-screen bg-[#070b14] text-white flex flex-col relative overflow-hidden">
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-6 text-center relative z-10">
          <div className="animate-slide-in-up max-w-2xl mx-auto">
            ...
            {/* Task 155: Hide "Iniciar Simulado" for Individual plan users - they should use their package */}
            {userPlan !== "individual" && (
              <button
                onClick={startQuiz}
                disabled={filteredQuestions.length === 0 || ((userPlan === "free" || !userPlan) && remainingQuestions === 0)}
                className="group relative px-12 py-5 bg-gradient-to-r from-orange-500 to-amber-500 rounded-2xl font-bold text-lg overflow-hidden transition-all duration-300 hover:scale-105 active:scale-95 shadow-xl shadow-orange-500/25 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
              >
